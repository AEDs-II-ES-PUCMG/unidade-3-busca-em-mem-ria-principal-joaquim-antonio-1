...
public class TabelaHash<K, V> implements IMapeamento<K, V> {
	...
	/// tabela que referenciará todas as listas lineares encadeadas. Nesse caso,
	/// estamos utilizando uma tabela hash com endereçamento em separado, ou seja, os
	/// itens são armazenados em listas lineares encadeadas.

	private int capacidade;
	/// tamanho da tabela hash. deve ser um número primo grande para diminuirmos a
	...
	public int getTamanhoPosicao(int posicao) {
		if (posicao < 0)
			throw new IllegalArgumentException("Posicao invalida");

		return tabelaHash[posicao].tamanho();
	}

	public Lista<V> getListaNaPosicao(int posicao) {
		Lista<Entrada<K,V>> lista;
		for (int i = 0; i < tabelaHash[posicao].getTamanho(); i++) {
			lista = tabelaHash[posicao];
	 ...
	 * @throws IllegalArgumentException no caso de um item, com a mesma chave, já
	 *                                  existir na tabela.
	 */
	@Override
	public int inserir(K chave, V item) {
		...
		/// adicionado. Se o item não for localizado, ele é inserido no final da lista
		/// encadeada associada à posição, da tabela hash, em que esse novo item
		/// será localizado.
		try {
	 ...
	 * @throws NoSuchElementException caso o item não seja localizado na tabela
	 *                                hash.
	 */
	@Override
	public V pesquisar(K chave) {
		...
		/// pesquisa o item, cuja chave foi passada como parâmetro para esse método, na
		/// lista encadeada associada à posição, da tabela hash, em que esse item deve
		/// estar armazenado.
	...
	public V remover(K chave) {
		...
		/// remove o item, cuja chave foi passada como parâmetro para esse método, da
		/// lista encadeada associada à posição, da tabela hash, em que esse item deve
		/// estar armazenado.
	 ...
	 * 
	 * @param acao Ação a ser executada para cada par chave/valor armazenado na
	 *             tabela.
	 */
